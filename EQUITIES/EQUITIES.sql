WITH user_fip_status AS (
  ...
  CASE 
    WHEN EXTRACT(ISODOW FROM CURRENT_DATE) = 0 THEN CURRENT_DATE - INTERVAL '2 day'  -- Sunday
    WHEN EXTRACT(ISODOW FROM CURRENT_DATE) = 1 THEN CURRENT_DATE - INTERVAL '3 day'  -- Monday
    ELSE CURRENT_DATE
          END
      )
),

...
denied_past_week AS (
    ...
    WHERE f.status IN ('ACTIVE', 'IN_PROGRESS')
      AND a.category = 'EQUITIES'
      AND a.data_status = 'DENIED'
      -- Fetch data denied in the past 7 days (excluding today & yesterday)
...
SELECT
    dt.user_id,
    dt.fip_id AS fip_denied_today,
    dt.masked_account_number AS account,
    CASE WHEN dy.user_id IS NOT NULL THEN 'YES' ELSE 'NO' END AS denied_yesterday,
    CASE WHEN dpw.user_id IS NOT NULL THEN 'YES' ELSE 'NO' END AS denied_in_past_week

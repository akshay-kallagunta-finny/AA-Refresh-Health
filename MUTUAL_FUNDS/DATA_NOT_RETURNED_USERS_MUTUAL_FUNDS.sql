WITH denied_today AS (
    ...
    WHERE f.status IN ('ACTIVE','IN_PROGRESS')
      AND a.category = 'MUTUAL_FUNDS'
	  AND a.data_status = 'DATA_NOT_RETURNED_IN_TIME'
...
SELECT
    dt.user_id,
    dt.fip_id AS fip_data_not_returned_in_time_today,
    dt.masked_account_number AS account,
    
    CASE WHEN dy.user_id IS NOT NULL THEN 'YES' ELSE 'NO' END AS data_not_returned_in_time_yesterday,
    CASE WHEN dpw.user_id IS NOT NULL THEN 'YES' ELSE 'NO' END AS data_not_returned_in_time_in_past_week

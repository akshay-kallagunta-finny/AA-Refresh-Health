WITH user_fip_status AS (
    ...
    CASE 
        -- If today is Sunday (7) → go back 2 days to Friday
        -- If today is Monday (1) → go back 3 days to Friday
        WHEN EXTRACT(ISODOW FROM CURRENT_DATE) = 0 THEN CURRENT_DATE - INTERVAL '2 day'
        WHEN EXTRACT(ISODOW FROM CURRENT_DATE) = 1 THEN CURRENT_DATE - INTERVAL '3 day'
        ELSE CURRENT_DATE
          END
      )
),
